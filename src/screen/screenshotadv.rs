...
pub fn capture_screenshot(secure_folder: &SecureFolder) -> io::Result<()> {
    // Start by trying to switch to interactive desktop
    if let Err(e) = switch_to_interactive_desktop() {
        error!("Warning: Failed to switch to interactive desktop: {}. Will try capture anyway.", e);
    }
    ...
    match capture_screen_gdi(&filename, secure_folder) {
        Ok(_) => {
            debug!("Successfully captured screen using GDI method");
            Ok(())
        }
        Err(primary_error) => {
            error!("Primary capture method failed: {}", primary_error);
            // Fall back to secondary method
            match capture_screen_dc(&filename, secure_folder) {
                Ok(_) => {
                    debug!("Successfully captured screen using DC method");
                    Ok(())
                }
                Err(secondary_error) => {
                    error!("Secondary capture method also failed: {}", secondary_error);
...
fn capture_screen_gdi(filename: &str, secure_folder: &SecureFolder) -> io::Result<()> {
    unsafe {
        ...
        // Convert to PNG and store in secure folder
        let dynamic_image = image::DynamicImage::ImageRgba8(image_buffer);
        let mut buffer = Vec::new();
        dynamic_image.write_to(&mut std::io::Cursor::new(&mut buffer), image::ImageFormat::Png)
            .map_err(|e| io::Error::new(io::ErrorKind::Other, e.to_string()))?;

        secure_folder.store_file(filename, &buffer)
            .map_err(|e| io::Error::new(io::ErrorKind::Other, e.to_string()))?;
        
        debug!("Screenshot captured and stored successfully");
        Ok(())
    }
}

fn capture_screen_dc(filename: &str, secure_folder: &SecureFolder) -> io::Result<()> {
    unsafe {
        ...
        debug!("Screenshot captured and stored successfully using DC method");
        Ok(())
    }
}